...
void ADC1_PB0_Init(void)
{
    // 1. Включаем тактирование порта GPIOB и модуля ADC1
    RCC->AHB1ENR |= RCC_AHB1ENR_GPIOBEN;
    RCC->APB2ENR |= RCC_APB2ENR_ADC1EN;

    // 2. Настраиваем пин PB0 в АНАЛОГОВЫЙ режим (11 в регистре MODER)
    GPIOB->MODER |= (3U << (0 * 2)); 
    
    // Отключаем внутренние подтяжки для аналогового пина
    GPIOB->PUPDR &= ~(3U << (0 * 2));

    // 3. Настройка параметров АЦП
    // Задаем время выборки (Sample Time) для 8-го канала. 
    // Поставим побольше циклов для стабильности (например, 56 циклов -> 011)
    ADC1->SMPR2 &= ~ADC_SMPR2_SMP8;
    ADC1->SMPR2 |= (3U << ADC_SMPR2_SMP8_Pos);

    // Указываем, что в регулярной последовательности у нас всего 1 канал (L[3:0] = 0000)
    ADC1->SQR1 &= ~ADC_SQR1_L;
    
    // Назначаем 8-й канал (PB0) первым и единственным в очереди преобразования
    ADC1->SQR3 &= ~ADC_SQR3_SQ1;
    ADC1->SQR3 |= (8U << ADC_SQR3_SQ1_Pos);

    // 4. Включаем АЦП (выставляем бит ADON)
