...
void SPI2_Init(void)
{
    ...
    // PA1 (RST), PA2 (DC) -> Выходы
    GPIOA->MODER   &= ~((3UL << (1 * 2)) | (3UL << (2 * 2)));
    GPIOA->MODER   |=  ((1UL << (1 * 2)) | (1UL << (2 * 2)));
    GPIOA->OSPEEDR |=  ((3UL << (1 * 2)) | (3UL << (2 * 2)));

    // PB12 (CS) -> Выход
    GPIOB->MODER   &= ~(3UL << (12 * 2));
    GPIOB->MODER   |=  (1UL << (12 * 2));
    GPIOB->OSPEEDR |=  (3UL << (12 * 2));

    // PB13 (SCK), PB15 (MOSI) -> Альтернативная функция AF5
    GPIOB->MODER   &= ~((3UL << (13 * 2)) | (3UL << (15 * 2)));
    GPIOB->MODER   |=  ((2UL << (13 * 2)) | (2UL << (15 * 2)));
    GPIOB->OSPEEDR |=  ((3UL << (13 * 2)) | (3UL << (15 * 2)));
    GPIOB->AFR[1]  &= ~((0xFUL << ((13 - 8) * 4)) | (0xFUL << ((15 - 8) * 4)));
    GPIOB->AFR[1]  |=  ((5UL << ((13 - 8) * 4)) | (5UL << ((15 - 8) * 4))); 
    ...
    // Настройка SPI2 (Master, Программный CS, частота PCLK1/4 = 12.5 МГц)
    SPI2->CR1 = 0;                        
    SPI2->CR1 &= ~(SPI_CR1_BR_Msk); // Сбрасываем старый делитель
    SPI2->CR1 |= SPI_CR1_MSTR | SPI_CR1_SSM | SPI_CR1_SSI | SPI_CR1_BR_0; // Делитель /4 (12.5 МГц)

    SPI2->CR1 |= SPI_CR1_SPE; 
}

void ST7735_SendByte(uint8_t byte) {
    while (!(SPI2->SR & SPI_SR_TXE));
    SPI2->DR = byte;
    while (SPI2->SR & SPI_SR_BSY); // <--- Внимание сюда!
}

// Чистая отправка байта без ручного дерганья CS
void ST7735_SPI2_RawSend(uint8_t byte)
{
    while (!(SPI2->SR & SPI_SR_TXE)); // Ждем пустой буфер передачи
    SPI2->DR = byte;                  // Пишем данные
}

// Отправка команды
void ST7735_WriteCmd(uint8_t cmd)
{
    while (SPI2->SR & SPI_SR_BSY);    // Ждем, если шина занята
    DC_L();                           // Режим команды
    CS_L();                           // Выбираем экран
    ST7735_SPI2_RawSend(cmd);
    while (SPI2->SR & SPI_SR_BSY);    // Ждем окончания физической отправки
    CS_H();                           // Отпускаем экран
}
...
void ST7735_WriteData(uint8_t data)
{
    ...
    DC_H();                           // Режим данных
    CS_L();
    ST7735_SPI2_RawSend(data);
    while (SPI2->SR & SPI_SR_BSY);
    CS_H();
}


void ST7735_HardReset_And_Init(void)
{
    // Аппаратный сброс ножкой
    RST_H();
    ...
    // ПРОГРАММНЫЙ СБРОС (Критично для неоригинальных чипов)
    ST7735_WriteCmd(0x01); 
    vTaskDelay(pdMS_TO_TICKS(120)); // Ждем перезапуска внутренней логики

    ST7735_WriteCmd(0x11); // Sleep Out (Выход из сна)
    vTaskDelay(pdMS_TO_TICKS(120));

    ST7735_WriteCmd(0x3A); // Interface Pixel Format
    ST7735_WriteData(0x05); // 16-бит/пиксель (RGB565)

    ST7735_WriteCmd(0x36); // Orientation
    ST7735_WriteData(0xC0); 

    ST7735_WriteCmd(0x29); // Display ON (Включить матрицу)
...
{
    // Обычные команды шлем в стандартном 8-битном режиме
    ST7735_WriteCmd(0x2A); 
    ST7735_WriteData(0x00); ST7735_WriteData(0x00); 
    ST7735_WriteData(0x00); ST7735_WriteData(127);  

    ST7735_WriteCmd(0x2B); 
    ST7735_WriteData(0x00); ST7735_WriteData(0x00); 
    ST7735_WriteData(0x00); ST7735_WriteData(159);  
    ...
    // --- РАЗГОН: Переключаем SPI2 в 16-битный режим (DFF = 1) ---
    SPI2->CR1 &= ~SPI_CR1_SPE; // Выключаем SPI перед сменой настроек
    SPI2->CR1 |= SPI_CR1_DFF;  // Включаем 16-битный формат данных
    SPI2->CR1 |= SPI_CR1_SPE;  // Снова включаем SPI
    
    // Гнать пиксели теперь в 2 раза быстрее, так как шлем по 16 бит за раз!
    for (uint32_t i = 0; i < 20480; i++) {
        while (!(SPI2->SR & SPI_SR_TXE)); // Ждем освобождения буфера
        SPI2->DR = color;                 // Шлем сразу весь пиксель целиком!
    }
    
    while (SPI2->SR & SPI_SR_BSY); // Ждем, пока улетит финальный пиксель
    
    // --- ВОЗВРАЩАЕМ 8-БИТНЫЙ РЕЖИМ (Критично для работы других команд) ---
    SPI2->CR1 &= ~SPI_CR1_SPE; 
    SPI2->CR1 &= ~SPI_CR1_DFF; // Сбрасываем в 8-битный режим
    SPI2->CR1 |= SPI_CR1_SPE;  
...
int16_t ucg_com_stm32_spi_cb(ucg_t *ucg, int16_t msg, uint32_t arg, uint8_t *data) 
{
  switch(msg)
  {
